...
from langgraph.checkpoint.memory import MemorySaver # per-thread
# import os
...
if __name__ == "__main__":
    ...
    cfg = {"configurable": {"thread_id": "reviews-BuffaloPhil"}}
    state1 = graph.invoke(init_messages, config=cfg)
    ans = state1["messages"][-1].content
    print(ans)

    follow_up = {"messages": [HumanMessage(content="Summarize it in 10 words or less.")]}
    state2 = graph.invoke(follow_up, config=cfg)
    print("-----Turn 2-----\n")
    ans = state2["messages"][-1].content
    print(ans)
